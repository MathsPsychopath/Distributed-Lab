...
func executor(bank *bank, executorId int, transactionQueue <-chan transaction, done chan<- bool, tasksAvailable semaphore.Semaphore) {
	for {
		t := <-transactionQueue
		
		from := bank.getAccountName(t.from)
		to := bank.getAccountName(t.to)
		//if locked, then do another one that isnt locked
		

		if bank.accounts[t.to].locked || bank.accounts[t.from].locked {
			// tasksAvailable.Wait()
		...
		bank.execute(t, executorId) //critical section
...
func main() {
	...
	tasksAvailable := semaphore.Init(3,3)
	done := make(chan bool)

	for i := 0; i < bankSize; i++ {
		go executor(&bank, i, transactionQueue, done, tasksAvailable)
	}
