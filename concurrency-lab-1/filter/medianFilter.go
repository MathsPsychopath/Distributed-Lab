...
func worker(startY, endY, startX, endX int, data func(y, x int) uint8, out chan<- [][]uint8) {
	filteredImage := medianFilter(startY,endY,startX,endX,data)
	out <- filteredImage
}

func useWorker(threads, quarterLength, width int, channelSlice []chan[][]uint8, immutableData func (y,x int) uint8){
	for i := 0; i < threads; i++ {
		channelSlice[i] = make(chan [][]uint8)
		go worker(i*quarterLength, (i+1)*quarterLength, 0, width, immutableData, channelSlice[i])
	}
}
...
func filter(filepathIn, filepathOut string, threads int) {
	...
	if threads == 1 {
		...
		channelSlice := [2]chan [][]uint8{}
		useWorker(threads, quarter, width, channelSlice[:], immutableData)
		for _, slice := range channelSlice{
			newPixelData = append(newPixelData, <-slice...)
		}
	} else if threads == 4 {
		channelSlice := [4]chan [][]uint8{}
		useWorker(threads, quarter, width, channelSlice[:], immutableData)
		for _, slice := range channelSlice{
			newPixelData = append(newPixelData, <-slice...)
		}
	} else if threads == 8 {
		channelSlice := [8]chan [][]uint8{}
		useWorker(threads, quarter, width, channelSlice[:], immutableData)
		for _, slice := range channelSlice{
			newPixelData = append(newPixelData, <-slice...)
		}
	} else if threads == 16 {
		channelSlice := [16]chan [][]uint8{}
		useWorker(threads, quarter, width, channelSlice[:], immutableData)
		for _, slice := range channelSlice{
			newPixelData = append(newPixelData, <-slice...)
		}
